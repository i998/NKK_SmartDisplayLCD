...
NKK Smart Display LCD 64*32 test code using library  NKK_SmartDisplayLCD 
 ...
 example 02- NKK as a library + Adafruit extension
 Status: works ok


// hardware setup for Arduino Pro Mini  
    // Arduino SS   (NCS)  <-->  10  -> NKK SS  (Signal is managed by NKK library to allow running several NKK devices with their own SS pins)
    // Arduino SCK  (CLK)  <-->  13  -> NKK SCK (Clock for serial communication, maximum 8 MHZ)
    // Arduino MISO (SPI)  <-->  12  -> NKK SDO (This is not required for NKK devices - no data output back from the NKK device) 
    // Arduino MOSI (SDO)  <-->  11  -> NKK SDI 
...
/* Note  - his example uses a local copy of the Adafruit_GFX library. 
Unfortunately by including a header file from a folder, Arduino will compile all folder's source files there  
(https://insigh.io/how-to-structure-a-project-to-be-compilable-by-arduino-ide-and-platformio-part-2/).
Adafruit_GFX library contains files which are not needed for NKK displays but they contain references to  
I2C and other objects which do not exist here and therefore cause compilation errors.  
In order to make this example and NKK_SmartDisplayLCD library lightweight and self-sustainable, use 
a local copy of the Adafruit_GFX library with the following five files removed from it: 
...
Alternatively, use full set of files from Adafruit_GFX library which will require Adafruit_BusIO library as well:
...
//Include extension to the Adafruit_GFX library which supports operations with the NKK_SmartDisplayLCD object:  
#include "Adafruit_GFX_Ext.h"



//Setup SPI
//Assuming built-in SPI object is already available and called SPI. No need for separate class instance. 
#define SPIDEVICE_CS 10 //note this is for the SPI setup only. Actual SS signal is managed by NKK library to allow running several NKK devices with their own SS pins.  
...
// Initialise NKK device
	  //Landscape
	NKK_SmartDisplayLCD NKK = NKK_SmartDisplayLCD(64,32,0,SPIDEVICE_CS,1000000); 
	  //Portrait,with 180 rotation
    //NKK_SmartDisplayLCD NKK = NKK_SmartDisplayLCD(32,64,1,SPIDEVICE_CS,1000000);

// Initialise Adafruit_GFX_EXT device
    
	Adafruit_GFX_Ext AGFXExt  = Adafruit_GFX_Ext(64,32, &NKK);
	
//	Adafruit_GFX_Ext AGFXExt(64,32, &NKK);
...
//test GFX image transfer  
  ...
  NKK.setColourNKK(255);  //White
  NKK.clearImageBufferGFX();
  
//test Adafruit GFX integration 
  AGFXExt.setCursor(3, 3);
  AGFXExt.print("test123456789");
  AGFXExt.drawCircle(50, 22, 8, 1); 
  AGFXExt.display();
  delay (5000);
